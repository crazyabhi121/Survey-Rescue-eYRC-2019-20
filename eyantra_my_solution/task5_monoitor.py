...
# Embedded file name: monitor_org_config.py
# Compiled at: 2020-02-27 15:49:53
...
class Led_Control():
    ...
    def __init__(self):
        ...
        self.stats_pub = rospy.Publisher('/stats_sr', SRDroneStats, queue_size=10)
        self.serviced_pub = rospy.Publisher('/serviced_info', SRInfo, queue_size=8)
        self.led_command_pub = rospy.Publisher('/led_control_topic', Int8, queue_size=8)
        self.base_cell_int_pub = rospy.Publisher('/base_cell_int_topic', Int8, queue_size=1)
        ...
        self.total_success_serviced_dict = {'FOOD': [], 'MEDICINE': [], 'RESCUE': []}
        ...
        self.patient_location = self.base_cell
    ...
    def verify_LED(self, msg):
        ...
        if cell_input_res:
            ...
            if current_coord not in self.cell_name_list:
                current_coord = None
    ...
    def publish_serviced_info(self, location, info):
        self.serviced_message.location = location
        ...
        if cell_input_res:
            ...
            if self.current_dest_cell not in self.cell_name_list:
                if self.current_dest_cell != self.base_cell:
                    ...
                    self.publish_serviced_info(msg.location, 'FAILURE')
    ...
    def write_tsv_config(self, file_path='LED_Config.tsv'):
        with open(file_path, mode='wb') as (outfile):
            config_writer = csv.writer(outfile, delimiter='\t', quoting=csv.QUOTE_MINIMAL)
    ...
    def stats_update_on_beacon(self):
        ...
        if self.led_module_status[self.current_led_module] == 'FOOD':
            ...
            self.set_total_success_serviced_dict('FOOD', self.current_dest_cell)
            ...
            self.set_total_success_serviced_dict('MEDICINE', self.current_dest_cell)
        elif self.led_module_status[self.current_led_module] == 'RESCUE':
            self.patient_onboard = True
            self.patient_location = self.current_dest_cell
    ...
    def stats_update_on_base(self):
        ...
        if self.patient_onboard == True:
            self.patient_onboard = False
            self.set_total_success_serviced_dict('RESCUE', self.patient_location)
            self.stats.correctServices = self.stats.correctServices + 1
            ...
            if all(map(operator.le, map(operator.abs, map(operator.sub, self.whycon_marker, self.current_setpoint)), self.thresh_val)):
                ...
                if self.total_time_cont > self.current_threshold_seconds and self.current_setpoint is not None:
                    ...
                    if self.base_cell == self.current_dest_cell:
                        ...
                        self.put_led_off_in_serial_queue(self.current_led_module, self.current_dest_cell)
                    temp_curr_dest = self.current_dest_cell
                    self.reset_current_variables_to_none()
                    self.publish_serviced_info(temp_curr_dest, 'SUCCESS')
            else:
                self.first_inside = True
    ...
    def set_total_success_serviced_dict(self, service_name, cell_name):
        string_repr_to_num = (ord(cell_name[0]) - 64) * 10 + int(cell_name[1])
        self.total_success_serviced_dict[service_name].append(string_repr_to_num)
        setattr(self.stats.totalSucessServiced, service_name, self.total_success_serviced_dict[service_name])
    ...
    def timeout_publish_led(self, led_module_index_in_led_seq_list, service_name, cell_name):
        if self.cell_name_list[led_module_index_in_led_seq_list] == self.current_dest_cell:
            self.reset_current_variables_to_none()
        self.stats.failedServices = self.stats.failedServices + 1
        led_module_no = self.led_seq_list[led_module_index_in_led_seq_list]
        self.serial_queue.put(led_module_no * 16)
        self.led_module_status[led_module_no] = 'OFF'
        self.publish_serviced_info(self.cell_name_list[led_module_index_in_led_seq_list], 'FAILURE')
    ...
    def put_led_off_in_serial_queue(self, led_module_no, cell_name):
        prev_led_status = self.led_module_status[led_module_no]
        if prev_led_status != 'OFF':
    ...
    def put_led_values_in_serial_queue(self, led_module_index_in_led_seq_list, service_name, cell_name):
        led_module_no = self.led_seq_list[led_module_index_in_led_seq_list]
        ...
        if service_name != 'OFF':
            ...
            t = threading.Timer(int(self.led_timeout_dict[service_name]), self.timeout_publish_led, [led_module_index_in_led_seq_list, service_name, cell_name])
    ...
    def light_leds_in_sequence(self):
        for idx, cell_name in enumerate(self.cell_name_in_timing_list):
            try:
                ...
                t = threading.Timer(float(self.led_time_delay_list[idx]), self.put_led_values_in_serial_queue, [led_module_index, self.led_service_list_names[idx], cell_name])
                self.timer_object_list.append(t)
                self.timer_object_dict[cell_name].append(t)
                t.start()
            except ValueError:
                print 'Beacon not mapped to LED module number'
                continue

    def reset_leds(self):
        if self.cell_name_list is not None:
            for idx, cell_name in enumerate(self.cell_name_list):
                self.put_led_off_in_serial_queue(self.led_seq_list[idx], cell_name)
    ...
    def hook(self):
        self.kill_sequence()
        self.publish_serviced_info(self.base_cell, 'END')
        print '\nShutting down monitor.pyc'


if __name__ == '__main__':
    print 'Original Config monitor.pyc ver. 1.5.4'
    rospy.init_node('monitor', anonymous=False, disable_signals=False)
    try:
        ...
        led_control.cell_name_list = [
         'B3', 'D4', 'E5', 'C5', 'A6', 'A2', 'C3', 'E2']
        led_control.led_seq_list = [2, 1, 0, 3, 4, 5, 6, 7]
        led_control.base_cell = 'E4'
        led_control.timer_object_dict_timeout = {'B3': [], 'C3': [], 'A2': [], 'A6': [], 'E5': [], 'D4': [], 'C5': [], 'E2': []}
        led_control.timer_object_dict = {'B3': [], 'C3': [], 'A2': [], 'A6': [], 'E5': [], 'D4': [], 'C5': [], 'E2': []}
        led_control.cell_name_in_timing_list = ['D4', 'E5', 'C3', 'E2', 'C5', 'A2', 'C5', 'D4', 'A6', 'E5', 'C3', 'C5', 'A2', 'B3', 'D4', 'E2', 'E5', 'A6', 'C5', 'A2', 'B3', 'D4', 'E2', 'C3', 'E5', 'A6', 'C5', 'A2', 'D4', 'B3', 'E2', 'E5']
        led_control.led_service_list_names = ['FOOD', 'MEDICINE', 'MEDICINE', 'RESCUE', 'FOOD', 'FOOD', 'RESCUE', 'MEDICINE', 'FOOD', 'MEDICINE', 'RESCUE', 'MEDICINE', 'FOOD', 'MEDICINE', 'RESCUE', 'MEDICINE', 'FOOD', 'FOOD', 'MEDICINE', 'MEDICINE', 'FOOD', 'RESCUE', 'FOOD', 'MEDICINE', 'MEDICINE', 'FOOD', 'FOOD', 'RESCUE', 'MEDICINE', 'MEDICINE', 'FOOD', 'RESCUE']
        led_control.led_time_delay_list = ['1', '1', '4', '10', '12', '14', '43', '32', '40', '43', '55', '62', '58', '62', '70', '75', '82', '85', '95', '93', '98', '104', '108', '115', '120', '122', '130', '134', '139', '143', '149', '155']
        led_control.load_cell_config_json(os.path.join(path, 'cell_coords.json'))
        for i in range(len(led_control.cell_name_list)):
            print 'LED no. ' + str(led_control.led_seq_list[i] + 1) + ' is at ' + led_control.cell_name_list[i] + (' with WhyCon coordinate of: {}').format(led_control.waypoint_dict[led_control.cell_name_list[i]])
        ...
        led_control.kill_timer = rospy.Timer(rospy.Duration(180), led_control.kill_node_event, oneshot=True)
